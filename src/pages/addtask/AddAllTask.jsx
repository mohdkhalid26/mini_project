...
function AddTask() {
    ...
    const createFunc=()=> {
        ...
        if(opt === ""  ) {
            ...
            setShoppingData([...shoppingData,text])
            setOpt("")
            setText("")
            setPopUp("success-show")
            setTimeout(()=>setPopUp("success-hide"),1000)
...
<div className="crossdiv">
    <img onClick={()=>navigate("/alltask")} src={cross} alt="" className="cross" />
</div>
...
<div onClick={()=>setCon1(!con1)} className='select' name="select a category" id="category"><span>{opt}</span></div>
  <div onClick={()=>setCon1(!con1)} className={con1?"opt-show" :"opt-hide"}>
    ...
    <img onClick={()=>setCon1(!con1)} style={{cursor:'pointer'}} src={arrow} alt="" />
